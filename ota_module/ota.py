...
# OTA_URL은 json 파일만 지원
#OTA_URL = 'https://raw.githubusercontent.com/JSShinnn/ota_test/main/version.json'
...
class OTAUpdater:
    ...
    def __init__(self, ssid, password, repo_url, filename):
        ...
        #서버쪽 버전 확인용 URL 선언
        #self.n_version_url = versionURL
        self.n_version_url = repo_url + 'version.json'
    ...
    def check_for_updates(self):
        ...
        #version_url = self.process_version_url(self.version_url, 'version.json')
        #version_url = self.version_url + 'version.json'
        
        response = urequests.get(self.n_version_url, headers=headers)
        #response = urequests.get(self.n_version_url, headers=headers)
